...
type VaultChrome = {
  ...
  badgeTitle?: string;
  /** Nested Local Vault status dot */
  dotClassName: string;
  dotTitle: string;
};

/**
 * Single resolver for Vault row attention + nested status dot.
 * Badge only for *actionable* secure-to-vault candidates (missing key files excluded by preview).
 */
function resolveVaultChrome(
  ...
  if (isVaultInUseError(error)) {
    return {
      attention: 'locked',
      badge: '!',
      badgeTitle: 'Vault open in another Zync window',
      dotClassName: 'bg-orange-500/80',
      dotTitle: 'Vault open in another Zync window',
    };
  }
  ...
  if (!vaultReady) {
    if (securableCount > 0) {
      return {
        ...
        dotClassName: 'bg-app-accent/70',
        dotTitle: 'Local vault not set up',
      };
    }
    return {
      attention: 'none',
  ...
  if (securableCount > 0) {
    return {
      ...
      dotTitle: `${securableCount} host${securableCount === 1 ? '' : 's'} ready to secure`,
    };
  }
  return {
    attention: 'ready',
    badge: null,
    dotClassName: 'bg-emerald-500/85',
    dotTitle: 'Local vault unlocked',
  };
}
...
export function VaultNavSection() {
  ...
  const chrome = useMemo(
    () => resolveVaultChrome(vaultStatus, vaultError, securableCount),
