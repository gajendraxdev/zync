...
function readAppCssColors(container: HTMLElement | null | undefined) {
  const computedStyle = getComputedStyle(container ?? document.body);
  return {
    appBg: computedStyle.getPropertyValue('--color-app-bg').trim(),
    appText: computedStyle.getPropertyValue('--color-app-text').trim(),
    appAccent: computedStyle.getPropertyValue('--color-app-accent').trim(),
  };
}
